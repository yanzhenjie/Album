...
Http请求框架：[https://github.com/yanzhenjie/NoHttp](https://github.com/yanzhenjie/NoHttp)  
...
* `Android6.0`运行时权限推荐使用：[AndPermission](https://github.com/yanzhenjie/AndPermission)，如果不使用6.0特性，可以把`targetSdkVersion`值设置的小于23，这样就不会使用`Android6.0`运行时权限了。
...
**如果你需要单选**，也就是点击相册中的图片就返回，不用勾选并确认，请使用单选模式：  
```java
Album.albumRadio(context)
    .toolBarColor(toolbarColor) // Toolbar 颜色，默认蓝色。
    .statusBarColor(statusBarColor) // StatusBar 颜色，默认蓝色。
    .navigationBarColor(navigationBarColor) // NavigationBar 颜色，默认黑色，建议使用默认。
    .title("图库") // 配置title。
    ...
    .start(999); // 999是请求码，返回时onActivityResult()的第一个参数。
```
和多选模式不同的是`Album.albumRadio(context)`，因为是单选，所以少了`selectCount()`和`checkedList()`方法。
...
Album.camera(context)
    ...
    .start(666); // 666是请求码，返回时onActivityResult()的第一个参数。
```
...
Album.gallery(context)
    ...
    .start(555); // 555是请求码，返回时onActivityResult()的第一个参数。
```
